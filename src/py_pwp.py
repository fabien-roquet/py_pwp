...
import py_pwp_functions as pypwp
import py_pwp_iotools as io
from py_pwp_default_params import params
import os
import sys
...
class py_mpwp_model():    
    ...
    def step(self):
        ...
        if sss_relax_switch:
            sss_d           = (sss_days*24.*60.*60.)/dt
            sss_timescale   = 1./sss_d
            SSS            = self.Srelax[0]
            salt           = pypwp.sss_relax(salt, SSS, ml_index, sss_timescale)
        ...
        if kt_switch==1:
            ...
            #u_star = u_star_l
            u_star = epsilon * U_a * np.sqrt( (A* cd_ice  + (1 - A) * cd_ocean ) * rho_air_ref / rho_ocean_ref )
        ...
        u[0:ml_index+1] += epsilon**2*(1+A*(cd_ice/cd_ocean-1))*tx/(ml_depth*density[0])*dt
        v[0:ml_index+1] += epsilon**2*(1+A*(cd_ice/cd_ocean-1))*ty/(ml_depth*density[0])*dt
